...
thredds = 'http://thredds.met.no/thredds/dodsC/arcticdata/met.no/tam24hNOgrd1957on/' \
          'tam24hNOgrd1957on_{0}_{1}_{2}.nc'  # .format(year,str(month).zfill(2),str(day).zfill(2)
...
def ll2utm33(lon, lat):
    ...
    :param lon: int/float, longitude east
    :param lat: int/float, latitude north
    :return: utm3_e,utm33_n
    """
...
site_offs = site_utms.copy().astype('int')
for i in range(len(seeds)):
...
while year < end_year+1:
    ...
    for seed_site in range(len(hdr)):
        site_day_T = nc_day.variables['mean_temperature'][0, site_offs[seed_site, 2],
                                                             site_offs[seed_site, 1]]
        out.append(str(round(float(site_day_T), 1)))
